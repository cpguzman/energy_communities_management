...
def rule_balance_power(model, gen, time, w):
  total_load = sum(model.Param_Pld_l_t[load, time] for load in model.LoadSet)
  GenPower = sum(model.VarP_OpGen_g_t[gen, time] + model.Var_r_UpGen_g_t_w[gen, time, w] for gen in model.GenSet)
  return model.VarP_Op_imp_t[time] - model.VarP_Op_Exp_t[time] + model.Var_r_Up_imp_t_w[time, w] - model.Var_r_Down_imp_t_w[time, w] == total_load - GenPower + model.Var_r_Down_imp_t_w[time, w]

model.balance_power_constraint = pyo.Constraint(model.GenSet, model.SetTimeIntervals, model.SetSceneries, rule=rule_balance_power)
#********************************************************************************************************
...
print(df_VarP_OpGen_g_t.sum(axis=0))
print(df_VarR_UpGen_g_t.sum(axis=0))
print(df_VarR_DownGen_g_t.sum(axis=0))
print(df_VarR_UpImp_t)
print(df_VarR_DownImp_t)
print(df_Var_r_UpGen_g_t_w)
print(df_Var_r_DownGen_g_t_w)
print(df_VarP_Op_imp_t)
print(df_Var_r_Up_imp_t_w)
print(df_Var_r_Down_imp_t_w)
print(df_VarP_Op_Exp_t)