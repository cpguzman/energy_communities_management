...
class BaseScene:
    ...
    def __init__(self, name: str, components: dict):
        ...
        self.darkened_colors = ['#0c7462', '#00276c', '#808804', '#12709e', '#993400', '#9e8212', '#a09c8a']
    ...
    def export_profile_to_excel(self,
        ...
        pImp = np.array(pImp, dtype=float)
    ...
    def plot (self,
        ...
        # Plot Production
        axs[0].fill_between(list(range(1, len(y1_prod)+1)), np.zeros(len(y1_prod)), y1_prod, color=self.project_colors_list[0], edgecolor=self.darkened_colors[0], linestyle="dotted", label="Generator Power Production")
        axs[0].fill_between(list(range(1, len(y2_prod)+1)), y1_prod, y1_prod + y2_prod, color=self.project_colors_list[1], edgecolor=self.darkened_colors[1], linestyle="dotted",label="BESS Discharging Power")
        axs[0].fill_between(list(range(1, len(y3_prod)+1)), y1_prod + y2_prod, y1_prod + y2_prod + y3_prod, color=self.project_colors_list[2], edgecolor=self.darkened_colors[2], linestyle="dotted",label="EV Discharging Power")
        axs[0].fill_between(list(range(1, len(y4_prod)+1)), y1_prod + y2_prod + y3_prod, y1_prod + y2_prod + y3_prod + y4_prod, color=self.project_colors_list[3], edgecolor=self.darkened_colors[3], linestyle="dotted",label="Load Reduction")
        axs[0].fill_between(list(range(1, len(y5_prod)+1)), y1_prod + y2_prod + y3_prod + y4_prod, y1_prod + y2_prod + y3_prod + y4_prod + y5_prod, color=self.project_colors_list[4], edgecolor=self.darkened_colors[4], linestyle="dotted",label="Load Cut")
        axs[0].fill_between(list(range(1, len(y6_prod)+1)), y1_prod + y2_prod + y3_prod + y4_prod + y5_prod, y1_prod + y2_prod + y3_prod + y4_prod + y5_prod + y6_prod, color=self.project_colors_list[5], edgecolor=self.darkened_colors[5], linestyle="dotted",label="Load ENS")
        axs[0].fill_between(list(range(1, len(pImp)+1)), y1_prod + y2_prod + y3_prod + y4_prod + y5_prod + y6_prod, y1_prod + y2_prod + y3_prod + y4_prod + y5_prod + y6_prod + pImp, color=self.project_colors_list[6], edgecolor=self.darkened_colors[6], linestyle="dotted",label="Grid Import Power")
        ...
        axs[1].fill_between(list(range(1, len(y1_cons)+1)), np.zeros(len(y1_cons)), y1_cons, color=self.project_colors_list[0], edgecolor=self.darkened_colors[0], linestyle="dotted",label="Load Power Consumption")
        axs[1].fill_between(list(range(1, len(y2_cons)+1)), y1_cons, y1_cons + y2_cons, color=self.project_colors_list[1],edgecolor=self.darkened_colors[1], linestyle="dotted", label="Grid Export Power")
        axs[1].fill_between(list(range(1, len(y3_cons)+1)), y1_cons + y2_cons, y1_cons + y2_cons + y3_cons, color=self.project_colors_list[2], edgecolor=self.darkened_colors[2], linestyle="dotted",label="BESS Charging Power")
        axs[1].fill_between(list(range(1, len(y4_cons)+1)), y1_cons + y2_cons + y3_cons, y1_cons + y2_cons + y3_cons + y4_cons, color=self.project_colors_list[3], edgecolor=self.darkened_colors[3], linestyle="dotted",label="EV Charging Power")
