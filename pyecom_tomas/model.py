...
def plot_profile(
    ...
    # True Production
    y1 = sum([result_genActPower.values[i].astype(float) for i in range(model.gen.last())]) 
    y2 = sum([result_storDchActPower.values[i] for i in range(model.stor.last())])
    y3 = sum([result_v2gDchActPower.values[i] for i in range(model.v2g.last())])
    y4 = sum([result_loadRedActPower.values[i] for i in range(model.loads.last())])
    y5 = sum([result_loadCutActPower.values[i] for i in range(model.loads.last())])
    y6 = sum([result_loadENS.values[i] for i in range(model.loads.last())])
    ...
    # True Consumption
    y1 = np.sum(Data.get_data().load['p_forecast'][:, model.t.first()-1:model.t.last()]*5, axis=0, dtype=np.float64)
    y2 = sum([result_genExcActPower.values[i] for i in range(model.gen.last())])
